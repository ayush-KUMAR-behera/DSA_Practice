# Stack Problems

## Problems Solved:

### Stack Implementation Using Array

Approach:
- Use an array to store stack elements
- Maintain a top pointer
- Perform push, pop, peek, search operations

Time Complexity:
- Push: O(1)
- Pop: O(1)
- Peek: O(1)
- Search: O(n)

Space Complexity: O(n)

Pattern: Stack (Array Implementation)

### Stack Implementation Using Linked List

Approach:

Use a linked list to store stack elements
Maintain a top pointer to the first node
Push inserts a node at the beginning
Pop removes the top node
Peek returns the top element without removing it
Search traverses the linked list to find an element

Time Complexity:

Push: O(1)
Pop: O(1)
Peek: O(1)
Search: O(n)
Display: O(n)

Space Complexity: O(n)

Pattern: Stack (Linked List Implementation)

Key Insight:

Stack follows LIFO (Last In First Out)
Linked List implementation supports dynamic size
No stack overflow due to fixed capacity
Push and Pop always happen at the head node


### Reverse String Using Stack

Approach:
- Push every character into the stack.
- Pop characters from the stack to get them in reverse order.
- Build the result using StringBuilder.

Time Complexity: O(n)

Space Complexity: O(n)

Pattern: Stack

### Remove All Adjacent Duplicates In String

Approach:
- Use a Stack to maintain characters.
- If the current character matches the stack top, pop it.
- Otherwise, push the character.
- Build the final string from the stack.

Time Complexity: O(n)

Space Complexity: O(n)

Pattern: Stack

### Valid Parentheses

Approach:
- Use Stack to store opening brackets.
- For every closing bracket, check the stack top.
- If brackets do not match, return false.
- At the end, stack must be empty.

Time Complexity: O(n)

Space Complexity: O(n)

Pattern: Stack


## Monotonic Stack

### Previous Greater
- Traverse left → right
- Remove `<= current`
- Stack top = previous greater
- Empty → `-1`

### Previous Smaller
- Traverse left → right
- Remove `>= current`
- Stack top = previous smaller
- Empty → `-1`

### Next Greater
- Traverse right → left
- Remove `<= current`
- Stack top = next greater
- Empty → `-1`

### Next Smaller
- Traverse right → left
- Remove `>= current`
- Stack top = next smaller
- Empty → `-1`

## Complexity

Time: `O(n)`  
Space: `O(n)`

**Pattern:** Monotonic Stack

### Daily Temperatures
- Traverse Right → Left
- Stack stores indices
- Remove temperatures `<= current`
- `stack top - current index` = waiting days
- Empty → `0`

## Complexity

Time: `O(n)`  
Space: `O(n)`

### Next Greater Element II

- Circular array problem.
- Traverse the array twice from right to left.
- Use a monotonic decreasing stack.
- Remove elements `<= current`.
- Stack top = next greater element.
- Empty stack → `-1`.

Time: O(n)
Space: O(n)

Pattern: Circular Monotonic Stack