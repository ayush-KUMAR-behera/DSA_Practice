...
- Use `isPossible()` to check if `k` cows can be placed with at least `mid` distance.
- If placement is possible, increase the distance to find a larger minimum distance.
...
- Apply Binary Search from index `0` to `rows * columns - 1`.
...
- If `nums[mid] < nums[mid + 1]`, move to the right half.
- Otherwise, move to the left half including `mid`.
- When `low == high`, the index is a peak element.
